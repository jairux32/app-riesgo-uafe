...
function AppContent() {
    ...
    const handleKeyDown = (e) => {
      if (e.ctrlKey || e.metaKey) {
        switch (e.key.toLowerCase()) {
          case 's':
            e.preventDefault();
            alert('Caso guardado en historial local');
            break;
          case 'enter':
            e.preventDefault();
          ...
          case 'p':
            e.preventDefault();
            if (step === 4) alert('Sugerencia: Use el botón Exportar PDF en pantalla');
            break;
        }
      }
    };
  ...
  const handleExpressAnalyze = () => {
    if (!datos.cliente || !datos.cedula) {
      alert('Por favor, complete los datos básicos del cliente primero');
      setStep(1);
      return;
    }
  ...
  const handleBatchAnalyze = async (selectedIds) => {
    if (!apiKey) {
      alert('Por favor, configure la API Key primero');
    ...
    try {
      for (let i = 0; i < targets.length; i++) {
        ...
        const resInherente = calculateInherentRisk(c.evaluaciones || {});
        const resResidual = calculateResidualRisk(resInherente.inherente, c.controlesEval || {});
        const prompt = buildPrompt(c.datos, resInherente, resInherente.factores, resResidual);
        const analysis = await analizarConGemini(apiKey, prompt);
        await updateCaseAnalysis(c.id, analysis);
      }
      alert(`Análisis por lotes completado para ${targets.length} casos.`);
    } catch (err) {
      alert('Error durante el análisis por lotes: ' + err.message);
    } finally {
